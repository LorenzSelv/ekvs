...
if __name__ == "__main__":
    ...
    if 5 in tests_to_run:
        try: # Test 5
            ...
            ## other_nodes is not defined in this test case
            other_nodes = [n for n in nodes if n not in part_nodes]
            ## d in not defined but is used for the causal_payload argument of the put request
            # d = send_put_request(hostname, other_nodes[0],  keys[0], 17, causal_payload=d['causal_payload'])
            ## change with empty payload
    ...
    def inspect_nodes(nodes):
        ...
        for i, node in enumerate(nodes):
            try: ## Node might be disconnected
                res = req.get('http://localhost:' + str(node.access_port) + '/kvs/debug')
                data = res.json()
                snapshot.append('===============')
    ...
    def snapshot_to_file(filename, nodes):
        with open('test/snapshots/%s.snap' % filename, "w") as f:
            f.write('\n'.join(inspect_nodes(nodes)))
    ...
    if 7 in tests_to_run:
        try: # Test 7
            ...
            snapshot_to_file('1partitions', nodes)
            ...
            snapshot_to_file('2partitions', nodes)
            ...
            for key in hm.keys():
                ...
                d = r.json()
                total_keys = total_keys + d["count"]                
            snapshot_to_file('3partitions', nodes)
            print(total_keys, len(keys))
            if total_keys != len(keys):
                raise Exception("Total number of keys in KVS are not equal to ones added initially")
            else:
